...
var AssignmentParser = (function(AssignmentParser, undefined) {
	
	// constants
	AssignmentParser.ASSIGNMENT_TABLE_PARSE_FUNCS = [
	...
	AssignmentParser.ASSIGNMENT_TABLE_KEYS = AssignmentParser.ASSIGNMENT_TABLE_PARSE_FUNCS.map(function (arr) { return arr[0]; });

	AssignmentParser.CATEGORY_TABLE_PARSE_FUNCS = [
		['name', null],
	...
	AssignmentParser.CATEGORY_TABLE_KEYS = AssignmentParser.CATEGORY_TABLE_PARSE_FUNCS.map(function (arr) { return arr[0]; });

	// matches "AVG 95.00%" or "AVG95.00%" in free variation
	AssignmentParser.COURSE_AVERAGE_REGEX = /avg\s?([\d\.]+)\%/i;
	...
	AssignmentParser.COURSE_NAME_REGEX = / {2,}(.*)$/;
	...
	function parseCourseGrade(str) {
		var matches = str.match(AssignmentParser.COURSE_AVERAGE_REGEX);
	...
	function parseCourseName(str) {
		var matches = str.match(AssignmentParser.COURSE_NAME_REGEX);
		if (matches && matches.length)
			return matches[1];
	 ...
	 */
	AssignmentParser.parse = function (doc) {
		return [].map.call($('.AssignmentClass', doc), function(elem, idx) {
			...
			if ($assignments.length)
				assignments = ParseUtils.getDataFromTable($assignments, AssignmentParser.ASSIGNMENT_TABLE_KEYS, true, false)
					.map(function (course) {
						return ParseUtils.parsePropertiesByKey(course, AssignmentParser.ASSIGNMENT_TABLE_PARSE_FUNCS);
					});

			// parse categories table
			var $categories = $course.find('.sg-content-grid .sg-asp-table-group .sg-asp-table');
			if ($categories.length)
				course_obj.categories = ParseUtils.getDataFromTable($categories, AssignmentParser.CATEGORY_TABLE_KEYS, true, true)
					.map(function (category) {
						return ParseUtils.parsePropertiesByKey(category, AssignmentParser.CATEGORY_TABLE_PARSE_FUNCS);
					});
			else
	 ...
	 * returns: [current, max]
	 */
	AssignmentParser.getMarkingPeriodInfo = function (doc) {
	 ...
	 */
	AssignmentParser.getASPState = function (doc) {
